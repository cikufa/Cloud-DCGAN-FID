...
class eval():
  def __init__(self, data_pth, datalist, latent_dim):
  ...
  def mu_sigma_calc(self, inception_model, evalbatch, n_samples, rf, generator):
    features_list = []
    cur_samples = 0
    ...
    #try:
      ...
      # while cur_samples < n_samples:
        if (rf == 0): #getin real image features  
          cropped, imn, real_samples, reallbl  = utils.generate_real_samples(cropped_images= cropped,
              img_num=imn, img_dir=self.data_pth, datalist=self.datalist, batch_size = evalbatch) #real sample: (evalbatch, 128*128 ,3)
